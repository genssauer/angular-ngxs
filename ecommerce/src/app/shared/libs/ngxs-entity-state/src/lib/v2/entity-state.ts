...
export abstract class EntityState<T> {
  ...
  protected constructor(storeClass: Type<EntityState<T>>, _idKey: keyof T) {
    ...
    this.setup(
      storeClass,
      Object.keys(EntityActionType).map((key) => EntityActionType[key])
    );
  }
  ...
  static get entities(): StateSelector<any[]> {
    const that = this;
    return (state) => {
      const subState = elvis(state, that.staticStorePath) as EntityStateModel<
        any
      >;
  ...
  update(
    ...
    if (payload.id === null) {
      affected = Object.keys(entities).map((key) => entities[key]);
    } else if (typeof payload.id === 'function') {
      affected = Object.keys(entities)
    ...
    if (typeof payload.data === 'function') {
      affected.forEach((e) => {
        entities = this._update(
          entities,
          (<Function>payload.data)(e),
          this.idOf(e)
        );
      });
    } else {
